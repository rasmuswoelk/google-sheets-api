...
A simple Google Sheets API wrapper that supports passing auth as a base64 string
...
(async function () {
  ...
  const googleSheetClient = new GoogleSheetsAPI({
    keyFile: process.env.KEY_FILE_BASE64, // Base64 encoded credentials JSON file
  });
  ...
  const result = await googleSheetClient.write(
    sheetId,
    tabName,
    range,
    dataToBeInserted
